...
USAGE: python cats_sync_variables.py
Roberta Volpe, Francesco Micheli, Jan Veverka, 31 October 2013.

TODO:
    * Add MET, diphoMVA, pho{1,2}_idMVA
'''
import math
...
## Will be prepended with {mu,ele}{1,2}_ for the hard, leading and subleading electron.
per_lepton_variables = '''
    pt
    eta
    phi
    '''.split()
leading_electron_variables  = ['ele1_' + var for var in per_lepton_variables]
trailing_electron_variables = ['ele2_' + var for var in per_lepton_variables]
leading_muon_variables      = ['mu1_'  + var for var in per_lepton_variables]
trailing_muon_variables     = ['mu2_'  + var for var in per_lepton_variables]
...
all_variables = (per_event_variables +
                 ...
                 per_diphoton_variables +
                 leading_muon_variables + 
                 trailing_muon_variables +
                 leading_electron_variables +
                 trailing_electron_variables +
...
def print_all_variables():
    ...
    print_variable_list(leading_muon_variables    )
    print "\nTrailing Muon Variables"
    print_variable_list(trailing_muon_variables    )
    
    print "\nLeading Electron Variables"
    print_variable_list(leading_electron_variables    )
    print "\nTrailing Electron Variables"
    print_variable_list(trailing_electron_variables    )
